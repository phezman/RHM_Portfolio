...
' Description
' ===========
' A VBScript to extract playlists from an iTunes XML file into a plain text or CSV file
...
Sub Action
 ...
 Open=False
  ...
  Set Artists=CreateObject("Scripting.Dictionary")
  Clock=0 : StartTimer
  ...
  Do While I<L
    ...
    If C="%" And I<(L-1) And IsHex(Mid(T,I+1,2)) Then
      ...
      If Err.Number<>0 Then         ' Handle error if one occurred
        Trace Null,"Problem processing item:" & T
        WScript.Quit
      End If
      I=I+2
...
Sub FileOpen
  ...
  Do 
    ...
    If Err.Number>0 Then
      R=MsgBox("Please close the file" & NL & Path,vbOKCancel,Title)
  ...
  StartTimer 
  Set File=FSO.CreateTextFile(Path,True,True)   ' Overwrite existing and create Unicode output
  Open=True
End Sub
